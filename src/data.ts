...
export const UPGRADES: UpgradeDefinition[] = [
  ...
  // === 融合技(全模式通用:先选任一基础技能即出池,改变攻击形态) ===
  {
    ...
    kind: "weapon",
    description: (level) =>
      `龙息喷火+护航无人机进化为金龙炼狱 · 按流派技能键召唤最多 4 个小无人机(15s 冷却,持续 8s):小型喷火可焚毁弹幕、不可暴击,优先攻击绝对血量最少敌人,基础伤害 ${level} + 敌人最大生命 0.04% · 龙息可暴击且伤害 +${35 + level * 15}% · 喷口每 0.22s 射出金龙火弹 · 龙息冷却 -${level}s`,
    short: (level) => `金龙无人机集群 + 龙息可暴击 +${35 + level * 15}%`
  },
  {
    id: "defender_fusion",
    name: "荆棘星垒",
    ...
    kind: "passive",
    description: (level) =>
      `荆棘护甲+旋刃进化为星垒 · 反伤提高 ${50 + level * 15}% · 每累计 400 点反伤触发星垒 2.5s:荆棘旋刃环绕,期间受击反伤 AOE(周围 220px 敌人每帧受最大生命 ${2 + level * 0.5}% 伤害) · 荆棘共鸣阈值减半`,
  ...
  },
  {
    ...
    kind: "passive",
    description: (level) =>
      `虹吸链+电弧进化为血星网络 · 链子吸血提高 15% · 每 0.5s 链头向最近 2 个敌人分支电弧(每支造成目标最大生命 ${0.5 + level * 0.1}% 伤害并吸血 20%) · 最多 8 条链共享网络`,
  ...
  },
  {
    ...
    kind: "passive",
    description: (level) =>
      `深渊吞噬+引力滞空航迹进化为星渊巨口 · 吞噬触发星渊巨口 2.5s:黑洞吸附 320px 内敌人并每帧造成最大生命 ${1.5 + level * 0.5}% 伤害 · 吸入的小兵直接吞噬(体型+回血) · 吞噬范围 +25%`,
